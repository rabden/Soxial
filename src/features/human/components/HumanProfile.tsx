...
export default function HumanProfile({ disabled = false }: { disabled?: boolean }) {
  ...
  const posts = usePaginatedList<HumanTweet>({
    resetKey: subTab,
    fetchPage: (until) => window.api.humanProfilePosts({ subTab, count: 10, until }),
      ...
      <div className="mx-auto max-w-[600px] border-x border-white/[0.06]">
        ...
        {!profileLoading && !authError && (
          <>
            ...
            {!posts.loading && posts.error && posts.error.category === 'auth' && (
              <AuthGate title="Log in to x.com to see your posts" onRecheck={recheck} checking={rechecking} />
            )}

            {!posts.loading && posts.error && posts.error.category !== 'auth' && (
              <div className="p-4">
                <OperationalError error={posts.error} onRetry={posts.reload} />
              </div>
            )}
            ...
            {posts.moreError && (
              <div className="p-4">
                <OperationalError error={posts.moreError} onRetry={posts.loadMore} />
              </div>
            )}
            ...
            {posts.hasMore && posts.items.length > 0 && !posts.moreError && (
              <div ref={posts.sentinelRef} className="h-px w-full" aria-hidden="true" />
            )}
