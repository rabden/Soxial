interface Window {
  api: {
    ...
    humanReply: (request: { tweetId: string; text: string; imagePaths?: string[] }) => Promise<import('../features/human/types').HumanResult<{ tweetId: string; replyTo: string; url?: string }>>
    humanReplyDraft: (request: import('../features/human/types').HumanReplyDraftRequest) => Promise<import('../features/human/types').HumanResult<import('../features/human/types').HumanReplyDraftResult>>
    pickReplyImages: () => Promise<{ ok: true; paths: string[] }>
    mediaDataUrl: (filePath: string) => Promise<{ success: boolean; dataUrl?: string; error?: string }>
